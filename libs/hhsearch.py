...
def create_database_from_pdb(fasta_path: str, databases: alphafold_classes.AlphaFoldPaths, output_dir: str) -> str:
    ...
    path_a3m = create_a3m(fasta_path, databases, database_dir)
    subprocess.call(['ffindex_build', '-as', f'{data_name}_a3m.ffdata', f'{data_name}_a3m.index', path_a3m])
...
def run_hhsearch(fasta_path: str, databases: alphafold_classes.AlphaFoldPaths, database_path: str,
                 output_path: str) -> str:
    path_a3m = create_a3m(fasta_path, databases, output_path)
    out = subprocess.Popen(['hhsearch', '-i', path_a3m, '-o', output_path, '-maxseq',
